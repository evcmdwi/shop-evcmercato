...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check apakah user dengan email ini exist
    ...
    // Tapi hanya kirim email kalau user exist
    if (userExists) {
      ...
      const { data, error } = await admin.auth.admin.generateLink({
        ...
        options: {
          redirectTo: `${
            process.env.NEXT_PUBLIC_SITE_URL || 'https://shop.evcmercato.com'
          }/reset-password`,
        },
      })
      ...
      if (!error && data?.properties?.action_link) {
        ...
        // From address: evie@evcmercato.com — domain evcmercato.com verified di Resend
        const fromAddress = 'Evie EVC <evie@evcmercato.com>'
