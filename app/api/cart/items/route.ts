...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existing) {
      ...
      if (updateError) {
        console.error('POST /api/cart/items update error:', updateError)
      ...
      if (insertError) {
        console.error('POST /api/cart/items insert error:', insertError)
    ...
    // Return full cart
    const fullCart = await getFullCart(cartId)
