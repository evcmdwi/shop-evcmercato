...
export async function GET(req: NextRequest) {
  ...
  let periodStart = searchParams.get('period_start')
  let periodEnd = searchParams.get('period_end')
  ...
  // Aggregate valid commissions in range
  const { data: commissions, error } = await admin
    ...
    .gte('valid_at', `${periodStart}T00:00:00.000Z`)
    .lte('valid_at', `${periodEnd}T23:59:59.999Z`)

  if (error) {
    console.error('[settlement/preview] Error fetching commissions:', error)
